...
@props([
    'items', 'selectedIds', 'id', 'label','class', 'name'
])
...
@php
    ...
    $selected_items = $items->reduce(function($items, $item) use($selected_ids){
        if( $item->children ) {
                ...
                <ul class="border rounded-md">
                    @forelse ( $items as $item)
                            ...
                            <li class="border-b border-gray-400">
                                ...
                                <ul class="ml-4">
                                    @foreach ($item->children as $child)
                                        <li class="border-b flex items-center dark:border-gray-600  {{ in_array($child->id, $selected_ids) ? 'text-gray-700 dark:text-gray-300 dark:bg-blue-800':'text-gray-900  dark:text-gray-300' }}">
                                            <label class="flex items-center px-2 py-2  w-full cursor-pointer">
                                                <input 
                                                type="checkbox" 
                                                name="{{ $name ?? '' }}" {{ in_array($child->id, $selected_ids) ? 'checked':'' }} 
                                                value="{{ $child->id }}"    
                        ...
                        @else
                            ...
                            <li class="border-b flex items-center dark:border-gray-600  {{ in_array($item->id, $selected_ids) ? 'text-gray-700 dark:text-gray-300 dark:bg-blue-800':'text-gray-900  dark:text-gray-300' }}">
                                <label class="flex items-center px-2 py-2  w-full cursor-pointer">
                                    <input 
                                    type="checkbox" 
                                    name="{{ $name ?? '' }}" {{ in_array($item->id, $selected_ids) ? 'checked':'' }} 
                                    value="{{ $item->id }}"    
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                    >
                                    <div class="ms-2 text-sm font-medium ">
